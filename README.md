...
- [Terai adapter](projects/terai.md): project-specific workflow rules for Terai, `tafs`, and the `terai_ye` demo reference.
...
- Stop for missing information (2026-06-26): whenever continuing correctly requires information, confirmation, credentials, or a decision only the user has, stop immediately, state the latest situation (what is done, where it is blocked, why the info is needed, and the impact of missing it), and wait for the user before proceeding. Do not guess past the gap or detour around it to keep busy. Self-serve first whatever code, command output, or existing docs can answer; only stop for what genuinely depends on the user. Once you have posed a question or choice to the user, never answer it on their behalf: if it is unanswered or skipped, wait for their reply — do not substitute your recommended option to keep moving (2026-06-26 user instruction).
...
- Real-model validation (2026-06-26): unit tests may use fakes/mocks, but any change touching LLM-facing behavior (provider, prompt, tool call, agent run, streaming/SSE) must include a real-provider smoke at the acceptance layer unless the environment is unreachable or the user waives it; each test plan must state which tests are deterministic fakes vs real-provider. Inject real-model endpoints/keys only via environment, untracked local config, or current user input.
- Secrets never persisted: never write credentials, tokens, cookies, private keys, or real user passwords into skills, docs, repos, logs, or task files; inject at runtime only and keep them out of version control.
