...
- Let `paos draft <parent>` call a configured LLM to generate structured Runtime Task Drafts for `ready-for-agent` Issues or approved Plans.
...
- No default LLM access to project source files or Private Runtime Logs.
- No Runtime implementation in this PRD artifact.
- No domain-level firewall or network egress enforcement in v1.

## User Stories

1. As a Personal Agent OS owner, I want `paos draft <parent>` to produce a model-drafted Runtime Task from a `ready-for-agent` Issue or approved Plan so that execution can be reviewed before it starts.
...
5. As a reviewer, I want failed execution to support a model-drafted follow-up Runtime Task instead of direct LLM edits so that repair remains auditable.
...
- `paos draft <parent>` calls a configured LLM and accepts only strict structured output; `paos` renders validated output into Runtime Task Markdown.
- LLM provider configuration lives in private local config such as `~/.personal-agent-os/config.yaml`; API keys are referenced through `api_key_env` and never stored in the Control Plane.
- The default LLM Payload includes the parent Issue or Plan, relevant Control Plane summaries, relevant ADR summaries, and Runtime Task templates.
- The default LLM Payload does not include Private Runtime Logs or project source files; adding those sources requires a separate approved design.
- `paos watch` observes local Control Plane changes; `paos scan` manually reconciles startup, missed events, and troubleshooting.
- `paos scan` is read-only and does not call the LLM or create Runtime Task Drafts.
...
- On failure, LLM Drafting may create a new `draft` Runtime Task from Control Plane result text that cites the failed task and log refs, but it must not edit files directly or read full Private Runtime Logs by default.
...
- [ ] A sample `ready-for-agent` Issue in this repository can be used by `paos draft <parent>` to produce a structured Runtime Task Draft under `.scratch/<feature>/runtime-tasks/`.
...
- [ ] The documented `paos` command surface includes `watch`, `scan`, `draft <parent>`, `approve <task>`, `run <task>`, and `status`.
- [ ] LLM Drafting rejects malformed or non-schema output instead of inferring intent from free-form text.
- [ ] The default LLM Payload excludes Private Runtime Logs and project source files.
...
- [ ] A failed execution path can produce a draft follow-up Runtime Task without direct LLM file edits.
...
- `docs/adr/0005-strict-llm-output-contract.md`: requires schema-constrained LLM output.
...
- `docs/adr/0014-llm-drafting-in-runtime-v1.md`: keeps LLM Drafting inside Runtime v1 but limited to draft artifacts.
...
- LLM Drafting sends Control Plane text to a model provider; mitigation is private provider config, `api_key_env`, strict LLM Payload boundaries, and no default source or private-log inclusion.
...
- LLM provider config is OpenAI-compatible and private: `base_url`, `model`, and `api_key_env`.
